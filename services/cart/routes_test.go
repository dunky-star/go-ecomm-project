...
var mockProducts = []payloads.Product{
...
func TestCartServiceHandler(t *testing.T) {
	...
	t.Run("should fail to checkout if the cart items do not exist", func(t *testing.T) {
		payload := payloads.CartCheckoutPayload{
			Items: []payloads.CartCheckoutItem{
...
func (m *mockOrderStore) CreateOrder(order payloads.Order) (int, error) {
	return 0, nil
}

func (m *mockOrderStore) CreateOrderItem(orderItem payloads.OrderItem) error {
	return nil
}